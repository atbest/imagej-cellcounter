...
public class CellCounter extends JFrame implements ActionListener, ItemListener{
    ...
    private void initializeImage(){
        ...
        if (imgTitle.startsWith("Counter Window - ")) {
            imgTitle = imgTitle.replaceAll("Counter Window - ", "");
            img.setTitle(imgTitle);
        }
        imgTitle = "Counter Window - "+img.getTitle();
        ...
        if (img.getStackSize() == 1) {
            ...
            counterImg = new ImagePlus(imgTitle, ip);
            ...
            counterImg = new ImagePlus(imgTitle, counterStack);
        ...
        /*
        JRadioButton rbutton = (JRadioButton)dynRadioVector.elementAt(index);
    	radioGrp.setSelected(rbutton.getModel(), true);
		currentMarkerVector = (CellCntrMarkerVector)typeVector.get(index);
		ic.setCurrentMarkerVector(currentMarkerVector);
        */
        int mtype = ic.getMarkerType();
        JRadioButton butt = (JRadioButton)(dynRadioVector.get(mtype));
        butt.setSelected(true);
        CellCntrMarkerVector defaultVector = (CellCntrMarkerVector)typeVector.get(mtype);
        ic.setCurrentMarkerVector(defaultVector);
    }
    ...
    public void loadMarkers(){
        ...
        if (storedfilename.startsWith("Counter Window - ")){
            storedfilename = storedfilename.replaceAll("Counter Window - ", "");
        }
    ...
    public void exportMarkers(){
        ...
        int currentMarkerType = typeVector.indexOf(currentMarkerVector);
        if (currentMarkerType < 0) {
            currentMarkerType = ic.getMarkerType();
        }
        wxml.writeXML(img.getTitle(), typeVector, currentMarkerType);
    }
